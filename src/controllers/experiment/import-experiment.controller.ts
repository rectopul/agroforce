        ...
        if (row !== '0') { // LINHA COM TITULO DAS COLUNAS
          ...
          if (experiment?.length > 0) {
            if (experiment[0].status !== 'IMPORTADO') {
              await logImportController.update({
                id: idLog, status: 1, state: 'INVALIDA', updated_at: Date().toLocaleString(), invalid_data: `Erro na linha ${Number(row) + 1}. Experimento já cadastrado e utilizado no sistema`,
              });
          ...
          if (experimentNameTemp.includes(experimentName)) {
            await logImportController.update({
              id: idLog, status: 1, state: 'INVALIDA', updated_at: Date().toLocaleString(), invalid_data: `Erro na linha ${Number(row) + 1}. Experimentos duplicados na tabela`,
            });
