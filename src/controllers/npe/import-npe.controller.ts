...
export class ImportNpeController {
  ...
  static async validate(
    idLog: number,
    { spreadSheet, idSafra, idCulture, created_by: createdBy }: ImportValidate
        ...
        if (row !== "0") {
          ...
          for (const column in spreadSheet[row]) {
              ...
              if (spreadSheet[row][column] !== null) {
                if (typeof spreadSheet[row][column] === "string") {
                  ...
                  if (response.total === 0) {
                    ...
                    const { response: responseSafra }: IReturnObject =
                      await safraController.getOne(idSafra);
            ...
            if (configModule.response[0]?.fields[column] === "Foco") {
              if (spreadSheet[row][column] !== null) {
                if (typeof spreadSheet[row][column] === "string") {
                  const { status: focoStatus, response }: IReturnObject =
                    await focoController.getAll({
                  ...
                  if (focoStatus !== 200) {
                    ...
                    const { status: focoGroup }: IReturnObject =
