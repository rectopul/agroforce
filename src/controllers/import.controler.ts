				...
				for (let column in spreadSheet[row]) {
					if (row === '0') {
						...
						if (spreadSheet[row][column] === null) {
							...
							let tec: any = await this.tecnologiaController.getAll({ id_culture: id_culture, cod_tec: this.aux.cod_tec });
				...
				try {
					for (let row in spreadSheet) {
						if (row !== "0") {
							...
							await this.tecnologiaController.create({ id_culture: response[0]?.id, name: spreadSheet[row][1], cod_tec: (spreadSheet[row][0].toString()), desc: spreadSheet[row][2], created_by: 23, dt_import: spreadSheet[row][4] });
						}
					}
					return "save"
				} catch (err) {
