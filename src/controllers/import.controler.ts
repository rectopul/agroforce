...
export class ImportController {
	...
	experimentoController = new ExperimentoController();
	...
	async responseIfErrorFactory(column: string, row: string, message: string): Promise<any> {
		return `<li style="text-align:left"> A ${Number(column) + 1}º coluna da ${row}º linha está incorreta, o campo ${message}. </li> <br>`;
	}
	...
	async createOrUpdateUnityLocal(unityCultureDTO: any) {
		const unidadeCulturaAlreadyExist = await this.unidadeCulturaController.getAll({ culture_unity_name: unityCultureDTO.culture_unity_name })

		if (unidadeCulturaAlreadyExist.response?.length > 0) {

			unityCultureDTO.id = unidadeCulturaAlreadyExist.response[0]?.id
			await this.unidadeCulturaController.update(unityCultureDTO)
	...
	async validateLocal({ spreadSheet, moduleId, id_safra, created_by }: object | any) {
		...
		try {
			...
			for (let row in spreadSheet) {
				for (let column in spreadSheet[row]) {
					if (row === '0') {
						...
						if (spreadSheet[row][column] === null) {
							responseIfError[Number(column)] += await this.responseIfErrorFactory(column, row, `${spreadSheet[0][column]} é obrigatório`)
						} else if (typeof (spreadSheet[row][column]) !== "number") {
							responseIfError[Number(column)] += await this.responseIfErrorFactory(column, row, `${spreadSheet[0][column]} deve ser numérico`)
						}
					}
						...
						if (spreadSheet[row][column] === null) {
							...
							if (safraYearValidate.response?.year !== spreadSheet[row][column]) {
								responseIfError[Number(column)] += await this.responseIfErrorFactory(column, row, `${spreadSheet[0][column]} não corresponde ao ano da safra selecionada`)
							} else if (typeof (spreadSheet[row][column]) !== "number") {
								responseIfError[Number(column)] += await this.responseIfErrorFactory(column, row, `${spreadSheet[0][column]} deve ser numérico`)
							}
						}
				...
				for (let row in spreadSheet) {
					if (row !== '0') {
						...
						try {
							...
							handleError('ImportController', 'Save local', error.message)
			...
			handleError('ImportController', 'Validate local', error.message)
	...
	async validateExperimento(data: object | any) {
		const responseIfError: any = [];
		let Column: number;
		let year_safra: any = 0;
		let year_lote: any;
		let id_experimento: any = 0;
		try {
			let configModule: object | any = await this.getAll(parseInt(data.moduleId));
				...
				for (const [keySheet, lines] of data.spreadSheet.entries()) {
					...
					for (const [sheet, columns] of data.spreadSheet[keySheet].entries()) {
						...
						if (keySheet != '0') {
							...
							if (configModule.response[0].fields[sheet] == 'id_experimento') {
								if (data.spreadSheet[keySheet][sheet] === "") {
									...
									id_experimento = data.spreadSheet[keySheet][sheet];
								}
							}

							if (configModule.response[0].fields[sheet] == 'experimento_name') {
								if (data.spreadSheet[keySheet][sheet] == "") {
									responseIfError[Column - 1] += `<li style="text-align:left"> A ${Column}º coluna da ${Line}º linha está incorreta, o campo identificador de dados do genótipo é obrigatorio.</li><br>`;
								} else {
							...
							if (configModule.response[0].fields[sheet] == 'id_dados_materiais') {
								if (data.spreadSheet[keySheet][sheet] == "") {
							...
							if (configModule.response[0].fields[sheet] == 'tratamentos') {
								if (data.spreadSheet[keySheet][sheet] != "") {
