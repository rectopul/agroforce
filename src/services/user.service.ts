...
const baseUrlPermissions = `${publicRuntimeConfig.apiUrl}/permissions`;
...
async function logoutSign(login: any, cultures: object | any) {
  ...
  setTimeout(() => 1000);
  await userPermissionService.update({ userId: user.response[0]?.id });
  setTimeout(() => 1000);
  await userPermissionService.update({ cultureId: cultures.selecionada, status: 1, idUser: user.response[0]?.id });
  setTimeout(() => 1000);
  localStorage.removeItem('user');
  setTimeout(() => 1000);
  userSubject.next(null);
...
async function initializePermissionsCall(){
  return fetchWrapper.get(`${baseUrlPermissions}/initializePermissions`, {});
}
