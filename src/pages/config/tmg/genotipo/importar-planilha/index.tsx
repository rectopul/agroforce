...
export default function Importar() {
  ...
  function readExcel(value: any) {
    ...
    readXlsxFile(value[0]).then((rows) => {
      importService.validate({ table: 'genotipo', spreadSheet: rows, moduleId: 10, id_culture: userLogado.userCulture.cultura_selecionada, id_safra: userLogado.safras.safra_selecionada, created_by: userLogado.id }).then((response) => {
        if (response.message !== '') {
          Swal.fire({
