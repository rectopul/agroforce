...
export default function Listagem({ allGenotipos, totalItems, itensPerPage, filterAplication, id_culture, id_safra, pageBeforeEdit, filterBeforeEdit }: IData) {
	...
	const formik = useFormik<IFilter>({
		...
		onSubmit: async ({ filterStatus, filterGenotipo, filterGenealogy, filterCruza }) => {
			const parametersFilter = `filterStatus=${filterStatus ? filterStatus : 1}&filterGenotipo=${filterGenotipo}&id_culture=${id_culture}&id_safra=${id_safra}&filterGenealogy=${filterGenealogy}&filterCruza=${filterCruza}`;
	...
	function columnsOrder(camposGerenciados: any): any {
		...
		Object.keys(columnCampos).forEach((_, index) => {
			...
			if (columnCampos[index] === 'gmr') {
				tableFields.push(headerTableFactory('GRM', 'gmr'));
			}
