...
export default function NovoLocal({ typeAssay, id_type_assay, id_safra, response, totalItens, itensPerPage, filterAplication, pageBeforeEdit }: IData) {
	...
	async function handleOrder(column: string, order: string | any): Promise<void> {
		...
		if (filter && typeof (filter) !== 'undefined') {
			...
			if (typeOrder !== '') {
				parametersFilter = `orderBy=${column}&typeOrder=${typeOrder}&id_safra=${id_safra}`;
			} else {
				parametersFilter = filter;
			}
		}
...
export const getServerSideProps: GetServerSideProps = async (context) => {
	...
	const filterAplication = `filterStatus=1&id_safra=${id_safra}&id_type_assay=${id_type_assay}`;
