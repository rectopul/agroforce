...
export default function Importar({ idCulture }: Idata) {
  ...
  function readExcel(value: any, idCulture: any,) {
    ...
    readXlsxFile(value[0]).then((rows) => {
      importService.validate({ table: 'tecnologia', spreadSheet: rows, moduleId: 8, id_culture: userLogado.userCulture.cultura_selecionada, createdBy: userLogado.id }).then((response) => {
        if (response.message !== '') {
          Swal.fire({
