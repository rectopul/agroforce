...
export default function Listagem({
  layouts, itensPerPage, filterApplication, totalItems, local, pageBeforeEdit, filterBeforeEdit,
}: Idata) {
  ...
  const formik = useFormik<IFilter>({
    ...
    onSubmit: async ({
      filterStatus, filterEsquema, filterDisparos, filterTiros, filterPlantadeira, filterParcelas,
    }) => {
      const parametersFilter = `filterStatus=${filterStatus || 1}&filterEsquema=${filterEsquema}&filterDisparos=${filterDisparos}&filterTiros=${filterTiros}&filterPlantadeira=${filterPlantadeira}&filterParcelas=${filterParcelas}`;
  ...
  async function handleStatus(idLayoutQuadra: number, data: any): Promise<void> {
    const parametersFilter = `filterStatus=${1}&id_culture=${userLogado.userCulture.cultura_selecionada}&esquema=${data.esquema}&status=${1}`;
    if (data.status == 0) {
