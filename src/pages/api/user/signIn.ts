...
function handler(req: NextApiRequest, res: NextApiResponse) {
	...
	async function authenticate() {
		...
		let permisions;
		let preferences: object | any = {};
		let userCulture: object | any = {};
		let safras: object | any = {};
		...
		if (user) {
			...
			preferences.group = await PreferencesControllers.getAllPreferences({ userId: user.id, module_id: 20 }); preferences.group = preferences.group.response[0];
