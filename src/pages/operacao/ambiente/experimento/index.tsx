...
export default function Listagem({
  ...
  async function handlePagination(page: any): Promise<void> {
    ...
    if (NPESelectedRow) {
      ...
      const finish = skip + Number(take);
      // let partial = npeData?.data.slice(skip, (skip + Number(take)));
      const partial = npeData?.data.slice(skip, finish);
  ...
  useEffect(() => {}, [npeDataItems, currentPage]);
  ...
  function validateConsumedData() {
    if (!SortearDisable) {
      ...
      Swal.fire({
        title: 'NPE Já usado !!!',
        html: `Existem NPE usados ​​entre <b>${npeUsedFrom}</b> e <b>${
          temp.npef
        }</b><br><br>${infos}O próximo NPE disponível é <strong>${
          Number(temp.nextAvailableNPE) + 1
        }</strong></div>`,
        icon: 'warning',
        showCloseButton: true,
                  ...
                  backgroundColor:
                    NPESelectedRow?.tableData?.id === rowData.tableData.id
                      ...
                      : rowData.npef >= rowData.nextNPE.npei_i
                        ? '#FF5349'
                        : checkValidAmbiente(rowData, allNPERecords)
                          ? '#f9fafb'
                          : '#ff8449',
                  height: 40,
                }),
