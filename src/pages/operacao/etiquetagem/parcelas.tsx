...
export default function Listagem({
  ...
  const CHILD_APP_URL =
    "operacao/etiquetagem/imprimir" ?? "http://localhost:3000";
  ...
  const [experimentGroupUpdated, setExperimentGroupUpdated] =
    useState<any>(experimentGroup);
  ...
  async function verifyAgain(inputCode: any) {
    ...
    if (colorVerify === "bg-green-600") {
      ...
      const parcelsToPrint = parcelsAgain.filter((parcela: any) =>
        parcelasToPrint.includes(parcela.id)
      );
  ...
  function onPressKey(e: any) {
    ...
    if (inputCode?.length < 9) {
      setErroMessage(
        "O código deve ser de 8 dígitos para baixa(NPE) e 13 para impressão(NCA)"
      );
    }
  ...
  async function validateInput() {
    ...
    if (inputCode?.length === 13) {
      ...
      if (doubleVerify) {
        if (functionsUtils?.generateDigitEAN13(inputCode) !== lastNumber) {
          setErroMessage(
            "O dígito verificador não confere com o NCA, favor clicar em limpar e tentar novamente"
          );
          setValidateNcaTwo("bg-red-600");
          return;
        }
    ...
    if (inputCode?.length === 8) {
      ...
      if (functionsUtils?.generateDigitEAN8(inputCode) !== lastNumber) {
        doubleVerify
          ? (setErroMessage(
              "O dígito verificador não confere com a NPE, favor clicar em limpar e tentar novamente"
            ),
            setValidateNcaTwo("bg-red-600"))
          : (setErroMessage(
              "O dígito verificador não confere com a NPE, favor clicar em limpar e tentar novamente"
      ...
      doubleVerify
        ? (setErroMessage(""), setValidateNcaTwo("bg-green-600"))
        : (setErroMessage(""), setValidateNcaOne("bg-green-600"));
                ...
                Toolbar: () => (
                  ...
                  >
                    ...
                    <strong className="text-blue-600">
                      Qte. exp: {experimentGroupUpdated.experimentAmount}
                    </strong>

                    <strong className="text-blue-600">
                      Total etiq. a imp.: {tagsToPrint}
                    </strong>

                    <strong className="text-blue-600">
                      Total etiq. imp.: {tagsPrinted}
                    </strong>
