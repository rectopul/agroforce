...
export default function Importar({ allItems, totalItems, itensPerPage, filterAplication }: IData) {
	...
	function readExcel(value: any, safra: any,) {
		...
		readXlsxFile(value[0]).then((rows) => {
			importService.validate({ spreadSheet: rows, moduleId: 22, safra: safra, created_by: userLogado.id }).then((response) => {
				if (response.message !== '') {
					Swal.fire({
