...
function validationCPF(cpf: any) {
  ...
  if (newCpf.length !== 11
    ...
    || newCpf === '77777777777'
    || newCpf === '88888888888'
    || newCpf === '99999999999') { return false; }
  // Valida 1o digito
  let add = 0;
  for (let i = 0; i < 9; i += 1) { add += Number(newCpf.charAt(i)) * (10 - i); }
  let rev = 11 - (add % 11);
  if (rev === 10 || rev === 11) { rev = 0; }
  if (rev !== Number(newCpf.charAt(9))) { return false; }
  // Valida 2o digito
  add = 0;
  for (let i = 0; i < 10; i += 1) { add += Number(newCpf.charAt(i)) * (11 - i); }
  rev = 11 - (add % 11);
  if (rev === 10 || rev === 11) { rev = 0; }
  if (rev !== Number(newCpf.charAt(10))) { return false; }
  return true;
}
